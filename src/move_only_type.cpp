...
public:
    ...
    Reservation(const Reservation& source) = delete;
    Reservation& operator=(const Reservation& source) = delete;
    ...
    [[nodiscard]] int id() const noexcept { return reservation_id_; }
...
int main() {
    ...
    std::cout << "  Alice valid: " << std::boolalpha
                << alice.valid()
                << ", id = " << alice.id() << '\n';
